...
export default function Editor(props) {
  ...
  useEffect(() => {
    setContent(props.shortStoryContent);
  }, [props.shortStoryContent]);

  // Delete
  const router = useRouter();
  const { deleteShortStory } = useContext(ShortStoriesContext);

  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`/api/shortStories/deleteShortStory`, {
        method: "POST",
        headers: {
          "content-type": "application/json",
        },
        body: JSON.stringify({ shortStoryId: props.id }),
      });
      const json = await response.json();
      if (json.success) {
        deleteShortStory(props.id);
        router.replace(`/shortStory/new`);
      }
    } catch (e) {}
  };
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const editResponse = await fetch(`/api/shortStories/editShortStory`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          shortStoryId: props.id,
          shortStoryContent: editedContent,
        }),
      });
...
export const getServerSideProps = withPageAuthRequired({
  async getServerSideProps(ctx) {
    ...
    const shortStory = await db.collection("shortStories").findOne({
      _id: new ObjectId(ctx.params.shortStoryId),
      userId: user._id,
    });
